...
``aiida-ssh2win`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-ssh2win`` repository (preferably using ``aiida-ssh2win`` as the project name)

The documentation is now available at `aiida-ssh2win.readthedocs.io <http://aiida-ssh2win.readthedocs.io/>`_.
